...
import rule from "../../src/rules/no-null-type-checked.js";
...
ruleTester.run("no-null-type-checked", rule, {
  valid: [
    {
  ...
  invalid: [
    // Type-aware (inference-based) cases.
    {
      ...
      // `noNull` from the `| null` type ref in the interface body, and
      // `noNullTypeChecked` from the binding's inferred type.
      errors: [
        { messageId: "noNull" },
    ...
    },
    {
      ...
      // `noNullTypeChecked` on the function declaration (return type contains
      // `null`), `noNull` on the literal `null` keyword in the return-type
      // annotation, `noNull` on the literal `null` value in the body, plus
      // `noNullTypeChecked` on the resulting `h` binding.
      errors: [
        { messageId: "noNullTypeChecked" },
        { messageId: "noNull" },
        { messageId: "noNull" },
    ...
    // Pure syntactic cases — the type-checked rule must still catch them all.
    {
      ...
      // `noNull` for the literal value, and `noNullTypeChecked` for the
      // binding's inferred type.
      errors: [
        { messageId: "noNullTypeChecked" },
        { messageId: "noNull" },
      ],
