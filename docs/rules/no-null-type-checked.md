# `no-null-type-checked`

Strict type-aware **superset** of [`no-null`](./no-null.md): in addition to
flagging every syntactic violation that `no-null` flags (the literal `null`
value and the `null` type keyword), this rule also asks the TypeScript type
checker whether a value's type is (or contains) `null` and reports it even
when no `null` literal or annotation appears at the use site.

Use this rule **instead of** [`no-null`](./no-null.md), not alongside it.
Because `no-null-type-checked` reports a strict superset of what `no-null`
...
- `enforce-effect/recommended` enables `no-null` (cheap, syntactic only).
- `enforce-effect/recommended-type-checked` enables `no-null-type-checked` and
  drops `no-null` from the same config.
...
This rule runs the same syntactic visitor as [`no-null`](./no-null.md) — so
the literal `null` value and the `null` type keyword are reported exactly as
they would be by the syntactic rule. On top of that, it visits each
value-bearing AST anchor and asks the TypeScript type checker for the type
at that position. It reports if the type is the intrinsic `null` type
(i.e. `type === checker.getNullType()`), or if it is a union,
...
check (for example, `const x: string | null = null` produces a `noNull`
report on the `null` annotation, a `noNull` report on the `null` value, and
a `noNullTypeChecked` report on the `x` binding). That overlap is
intentional: each report explains a different reason the value is
problematic.
...
### Syntactic violations (also caught by `no-null`)

The literal `null` value and the `null` type keyword are flagged exactly as
in the syntactic [`no-null`](./no-null.md) rule:

```ts
const value = null;                 // noNull (and noNullTypeChecked)
type MaybeValue = string | null;    // noNull
```
...
[`no-null`](./no-null.md) alone instead.
...
```ts
// eslint-disable-next-line no-null-type-checked -- DOM ref is null until React mounts;
...
- [`no-null`](./no-null.md) — the cheaper syntactic-only sibling. Use one or
  the other, not both.
- [`no-undefined-type-checked`](./no-undefined-type-checked.md) — the parallel
  rule for `undefined`.
