      ...
      CallExpression(node) {
        ...
        if (
          ...
          subtreeHas(
            ...
            (candidate) => candidate !== nestedArgument && isEffectCall(candidate),
          )
        ) {
          context.report({
            node,
  ...
  "no-effect-wrapper-alias": createEffectOnlyRule(
    ...
    (context) => ({
      ...
      FunctionDeclaration(node) {
        if (
          node.body &&
          textIncludesAny(context.sourceCode.getText(node.body), ["Effect.", "pipe("])
        ) {
          context.report({
            node,
            messageId: "noEffectWrapperAlias",
    ...
    (context) => ({
      CallExpression(node) {
        if (
          ...
          subtreeHas(
            node.arguments,
            (candidate) => candidate !== node && isNamespacedCall(candidate, "Effect", "flatMap"),
          )
        ) {
          context.report({
            node,
    ...
    (context) => ({
      CallExpression(node) {
        if (
          ...
          const text = context.sourceCode.getText(node);
          if (text.includes("Match.when") && branchContainsSequencing(context, node)) {
            context.report({
              node,
        ...
        if (isNamespacedCall(node, "Option", "match") && branchContainsSequencing(context, node)) {
          context.report({
            node,
            messageId: "noMatchEffectBranch",
          });
    ...
    (context) => ({
      CallExpression(node) {
        ...
        if (
          ...
          subtreeHas(
            firstArgument,
            (candidate) => candidate !== node && isNamespacedCall(candidate, "Effect", "gen"),
          )
        ) {
          context.report({
            node,
    ...
    (context) => ({
      CallExpression(node) {
        ...
        if (
          ...
          !(
            firstArgument.body.callee.type === AST_NODE_TYPES.MemberExpression &&
            firstArgument.body.callee.object.type === AST_NODE_TYPES.Identifier &&
